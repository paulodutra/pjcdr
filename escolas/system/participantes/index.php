    ...
    <table id="school" class="table table-striped table-hover dt-responsive container" cellspacing="0" width="100%">
      <thead>
       ...
       <tbody>
        ...
        $readParticipant->FullRead("SELECT * FROM `es_school_participant` AS p INNER JOIN es_school AS S ON participant_school=school_id INNER JOIN es_school_participant_type AS t ON participant_type_id=participant_type WHERE school_id={$userSchool['school_id']} ORDER BY participant_id ASC");
      ...
      ?>
          ...
          MSGErro("<p class=\"text-center\"><b>A escola não possui participantes cadastrados !</b></p>", MSG_ALERT);
