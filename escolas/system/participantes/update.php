<?php
	...
	*/
	...
	$participanteID= filter_input(INPUT_GET,'participante',FILTER_VALIDATE_INT);

	$disable=(!empty($participanteID) ? 'disabled' :'');
	...
	if($participant && isset($participant['sendParticipant'])):
		...
		$createParticipante->ExeUpdate($participanteID,$participant);
		...
		$readParticipant->ExeRead("es_school_participant","WHERE participant_id=:id","id={$participanteID}");
		//$readParticipant->getResult()[0]['participant_date_nascimento']=date('d/m/Y',strtotime($readParticipant->getResult()[0]['participant_date_nascimento']));

		if($readParticipant->getResult()):			
			
			$participant=$readParticipant->getResult()[0];
            ...
            <div class="row">
            	<h4 class="text text-primary text-center">Cadastro de Participantes do Concurso</h4>
            	 ...
            	 <div class="form-group col-lg-10 ">
	              	<label>Escola:</label>
	                	<select  class="form-control" disabled="disabled" name="participant_school" required>
	                    	 ...
	                    	 <?php
		             			...
		             			if($readSchool->getResult()):
		             				foreach ($readSchool->getResult() as $school): 
		             					...
		             					if(isset($participant['participant_school']) && $participant['participant_school']==$school_id):
		             						 echo 'selected';
		             					endif;		
            	...
            	<div class="form-group col-lg-7">
            		<div class="form-group col-lg-4">
            			...
            			<input type="text" class="form-control" name="participant_date_nascimento" id="nascimento" value="<?php if(isset($participant['participant_date_nascimento'])): echo date('d/m/Y',strtotime($participant['participant_date_nascimento'])); endif; ?>" required>
            		</div>
            		<div class="form-group col-lg-3">
            			<label>CPF:</label>
