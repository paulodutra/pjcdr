...
<div class="dt-empresa">
    <table id="school" class="table table-striped table-hover dt-responsive container" cellspacing="0" width="100%">
        ...
        <tbody>
            <?php
            $readPhone = new Read();
            $readPhone->FullRead("SELECT * FROM es_school INNER JOIN es_school_phone ON school_id=phone_school INNER JOIN es_phone_type ON phone_type = type_id WHERE school_id={$userSchool['school_id']} ORDER BY school_id ASC ");
                ...
                foreach ($readPhone->getResult() as $schoolphone):
                    ...
                    <tr>
                        ...
                        <td class="text-center">
                            <span><a href="dashboard.php?exe=phone/details&school=<?= $school_id ?>" class="glyphicon glyphicon-search" title="Visualizar Telefone(s)"></a></span>
                            <span><a href="dashboard.php?exe=phone/create&school=<?= $school_id ?>" class="glyphicon glyphicon-phone-alt" title="Cadastrar Telefone"></a></span>
                            <span><a href="dashboard.php?exe=phone/update&school=<?= $phone_school ?>&phone=<?= $phone_id ?>" class="glyphicon glyphicon-pencil" title="Editar Telefone"></a><span>
                                ...
                                MSGErro("<p class=\"text-center\"><b>A escola não possui telefones cadastrados !</b></p>", MSG_ALERT);
                            endif;
                            ?>
                            </tbody>
