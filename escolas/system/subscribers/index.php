    ...
    <table id="school" class="table table-striped table-hover dt-responsive container" cellspacing="0" width="100%">
        ...
        <tbody>
            ...
            $readSubscribers->FullRead("SELECT * FROM cs_concurso_subscribers INNER JOIN cs_concurso ON subscribers_concurso=concurso_id
                                    ...
                                    INNER JOIN cs_series ON series_id=subscribers_series WHERE school_id=
            ...
            if ($readSubscribers->getResult()):
                foreach ($readSubscribers->getResult() as $subscribers):
                    ...
                    <tr>
                        ...
                        <td class="text-center">
                            <span><a class="glyphicon glyphicon-search" href="dashboard.php?exe=subscribers/details&school=<?= $school_id ?>&student=<?= $subscribers_student ?>" class="text-primary " title="Visualizar Inscrição"></a></span>            
                ...
                MSGErro("<p class=\"text-center\"><b>A escola não possui inscrições cadastradas !</b></p>", MSG_ALERT);
