...
class AdminConcurso{
	...
	*/
    ...
    private function setName(){
    	...
    	$readName= new Read();
    	$readName->ExeRead(self::Entity,"WHERE {$Condition} concurso_start LIKE '%$Year%' AND concurso_type=:type  ","type={$this->Data['concurso_type']}");
    	
    	$readType= new Read();
    	$readType->ExeRead('cs_concurso_type',"WHERE type_id =:type","type={$this->Data['concurso_type']}");
    	
    	if($readName->getResult()):
    		$Number=count($readName->getResult());
    		$Number=$Number+1;
    		$this->Data['concurso_name']=$Number.'° - Concurso de '.$readType->getResult()[0]['type_name'].' '.$Year;
    	else:			
    		$this->Data['concurso_name']='1°- Concurso de '.$readType->getResult()[0]['type_name'].' '.$Year;
    	endif;	
    ...
    */
    ...
    private function Create(){
    	...
    	if($createConcurso->getResult()):
    		...
    		$this->Result = $createConcurso->getResult()[0]['concurso_id'];
