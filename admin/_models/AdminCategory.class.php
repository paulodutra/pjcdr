...
class AdminCategory {
    ...
    public function ExeDelete($CategoryID) {
        ...
        if ($this->Result):
            ...
            $this->Result = false;
            $this->Error = ["<b>Erro ao deletar:</b> A categoria informada possui series cadastradas. Por favor exclua a serie(s) antes de deletar a categoria!", MSG_ERROR];
    ...
    /**
     * <b>setCategorySeries</b> Método responsável por verificar se a categoria possui series adicionadas. Caso exista a mesma restorna 
     * $this->Result=false, caso contrario retorna $this->Result=true. Este metodo é utilizada antes de deletar uma categoria do sistema.
     */
    private function setCategorySeries() {

        $readCategorySeries = new Read();
        $readCategorySeries->FullRead("SELECT * FROM cs_category INNER JOIN cs_category_series ON category_id=category_category WHERE category_id=:id", "id={$this->CategoryID}");

        if ($readCategorySeries->getResult()):
            $this->Result = false;
        else:
            $this->Result = true;
        endif;
