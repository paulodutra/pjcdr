...
<div class="dt-empresa">
    <table id="school" class="table table-striped table-hover dt-responsive container" cellspacing="0" width="100%">
        ...
        <tbody>
            <?php
            $readBank = new Read();
            $readBank->FullRead("SELECT * FROM es_school INNER JOIN app_estados AS e ON school_uf=e.estado_id INNER JOIN app_cidades AS c ON school_city=c.cidade_id INNER JOIN es_school_data_bank ON data_bank_school=school_id INNER JOIN cs_concurso_bank ON data_bank_bank=bank_id");
