...
$bankID= filter_input(INPUT_GET,'bank', FILTER_VALIDATE_INT);

if (isset($bank) && $bank['sendBank']):
    unset($bank['sendBank']);
    $createBank = new AdminBank();
    $createBank->ExeUpdate($bankID,$bank);
    var_dump($createBank);
    ...
    $readBank= new Read();
    $readBank->ExeRead('es_school_data_bank',"WHERE data_bank_id=:id","id={$bankID}");
    if($readBank->getResult()):
        $bank= $readBank->getResult()[0];
    endif;
