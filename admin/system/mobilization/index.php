...
<div class="dt-empresa">
    <table id="school" class="table table-striped table-hover dt-responsive container" cellspacing="0" width="100%">
        ...
        <tbody>
            ...
            $readMobilization->FullRead("SELECT * FROM `es_school_mobilization` INNER JOIN es_school ON mobilization_school=school_id INNER JOIN app_cidades AS c ON c.cidade_id=school_city INNER JOIN app_estados AS e ON e.estado_id=school_uf INNER JOIN cs_concurso ON concurso_id=mobilization_concurso");
            if ($readMobilization->getResult()):
                foreach ($readMobilization->getResult() as $mobilization):
                    ...
                    <tr>
                        ...
                        <td>
                            <span><a href="dashboard.php?exe=mobilization/details&school=<?= $school_id ?>&concurso=<?= $concurso_id ?>" class="glyphicon glyphicon-search"></a></span>
                            <span><a href="dashboard.php?exe=mobilization/update&mobilization=<?= $mobilization_id ?>" class="glyphicon glyphicon-pencil"></a></span> 
                            <span><a href="#" class="glyphicon glyphicon-trash"></a></span>
                        </td>
                    </tr>
                    <?php
                endforeach;
            else:
                MSGErro("Não há mobilizações cadastradas", MSG_ALERT);
            endif;
            ?>
